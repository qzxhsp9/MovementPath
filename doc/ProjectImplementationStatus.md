...
当前 lazy 诊断字段包括距离计算、距离改善/未改善、状态写入/未变化写入、Occupied/ClearanceBand 未变化写入拆分、chunk candidate triangle min/max 分布和 influence cache hit/miss。benchmark 支持 `--runs N` 多轮输出，CSV 使用 `runIndex` / `runCount` 标识轮次。
